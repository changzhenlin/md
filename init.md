...
1. **按实施计划分步开发**
   - **每次新功能/步骤开始前**：
     ...
     - 切换到 Plan 模式（根据步骤来定）。
   - **执行指令示例**：\
     “请基于 @memory-bank/implementation-plan.md 中的 \[步骤编号：Step 2.1] 进行开发, 任务完成后，请同步更新 @memory-bank/architecture.md（如有结构变动）和 @memory-bank/progress.md。必须遵守 CLAUDE.md 中的所有规则。”
   - **AI 生成**：代码、测试、文档更新。
